...
class Note {
    ...
    static async create(data) {
        ...
        const response = await db.query("INSERT INTO Notes (user_id, title, context, created_at, updated_at) VALUES ($1, $2, $3, $4, $5) RETURNING *;", [user_id, title, context, date, updated_at]);
    ...
    static async update(data, id) {
        ...
        const response = await db.query("UPDATE Notes SET user_id = $1, title = $2, context = $3, updated_at = $4 WHERE note_id = $5 RETURNING *;",
            [user_id, title, context, date, id]);
        if (response.rows.length != 1) {
